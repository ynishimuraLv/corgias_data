...
for lin in lineages:
    ...
    for th in thresholds:
        tmp = df_ppv[lin]
        asa = set(tmp['asa'][th]['COG_pair'])
        sev = set(tmp['sev'][th]['COG_pair'])
        GLDistance = set(tmp['GLDistance'][th]['COG_pair'])
        all_methods = asa.intersection(sev).intersection(GLDistance)
        GLDistance_sev = GLDistance.union(sev).difference(all_methods)
        sev_asa = sev.intersection(asa).difference(all_methods)
        asa_GLDistance = asa.intersection(GLDistance).difference(all_methods)
        only_GLDdistance = GLDistance.difference(sev.union(asa))
        only_sev = sev.difference(GLDistance.union(asa))
        only_asa = asa.difference(sev.union(GLDistance))
        heatmaps[lin].append(
            [ len(subset) for subset in [all_methods, GLDistance_sev, sev_asa, asa_GLDistance,
                                          only_GLDdistance, only_sev, only_asa]])
    heatmaps[lin] = pd.DataFrame(heatmaps[lin]).T
    heatmaps[lin] = heatmaps[lin].rename(
                      index = {0:'ASA & SEV & GLDistance', 1:'SEV & GLDistance only', 2:'ASA & SEV only',
                               3:'ASA & GLDistance only', 4:'GLDistance only', 5:'SEV only', 6:'ASA only'},
                      columns = {0:'0.9', 1:'0.8', 2:'0.7', 3:'0.6', 4:'0.5'})
...
for lin in lineages:
    ...
    for th in thresholds:
        ...
        only_sev = sev.difference(GLDistance.union(asa))
        only_asa = asa.difference(sev.union(GLDistance))
        subsets = [all_methods, GLDistance_sev, sev_asa, asa_GLDistance,
                   only_GLDdistance, only_sev, only_asa]
        total = 0
        for subset in subsets:
            total += len(subset)
