...
# %%
lineage = ['pseudomonadales', 'mycobacteriales', 'archaea']
df = {}
for lin in lineage:
...
# %%
print('lineage total weighted transition only_transition, only_weighted')
for lin in df_weighted.keys():
    weighted = set(df_weighted[lin]['COG_pair'])
    transtion = set(df_transition[lin]['COG_pair'])
    print(lin, len(weighted.union(transtion)), len(weighted), len(transtion), len(transtion.difference(weighted)), len(weighted.difference(transtion)))

# %%
print('lineage total weighted transition only_transition only_weighted')
for lin in df_weighted.keys():
    weighted = set(df_weighted[lin]['COG_pair'])
    transition = set(df_transition[lin]['COG_pair'])
    total = len(weighted.union(transition))
    print(lin, len(weighted)/total, len(transition)/total, 
          len(transition.difference(weighted))/total, len(weighted.difference(transition))/total)
